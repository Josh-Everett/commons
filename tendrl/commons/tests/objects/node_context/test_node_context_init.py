...
class MockGlusterBrick(object):
    def __init__(self, integration_id = "1", fqdn="127.0.0.1"):
        self.status = ""
...
class MockCNC(object):
    def __init__(self, node_id=1, integration_id="1", is_managed="yes", first_sync_done = "yes"):
        self.is_managed ="yes"
...
#@patch.object(cluster_node_context.ClusterNodeContext, "load")
...
def test_on_change(patch_get_node_id,
    ...
    with patch.object(etcd.Client, "read", return_value=etcd.Client()):
        node_context = NodeContext()
        node_context.on_change("status", "test_prev_value", current_value=None)
    with patch.object(etcd.Client, "read", return_value=etcd.Client()):
        node_context = NodeContext()
        node_context.on_change("status", "test_prev_value", current_value="UP")
